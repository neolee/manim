...
#
# Part 1. 编写程序绘制 Mandelbrot set，要求：
# 1. 可以用鼠标选择一个局部进行放大重绘；
# 2. 可以通过命令行参数选择不同的调色盘；
# 3. 可以通过命令行参数输出图像文件（PNG格式）；
# 4. 尽可能优化性能。
#
# Part 2. 增加更多好看一点的 palette 吧
...
def gradient_palette(
    ...
    *,
    ...
    基于控制点的颜色渐变调色盘。
    colors: RGB in [0,1]
    positions: 每个颜色对应的 x∈[0,1] 位置，若 None 则均匀分布。
    gamma: 对输入 x 做 x**gamma。
    repeat: 如果 True，则对 x 做周期折返（不过当前 x 已在[0,1]，主要用于扩展）。
    """
    N = len(colors)
    assert N >= 2, "colors 至少需要 2 个控制点"
    ...
    def mapper(x: np.ndarray) -> np.ndarray:
        x = _clamp01(x.astype(np.float32))
        if repeat:
            x = x % 1.0
        if gamma != 1.0:
            x = np.power(x, gamma, dtype=np.float32)

        # 对每个 x 找到所在的段 [P[i], P[i+1]]
        # searchsorted 返回插入位置，减一即为左段索引
...
# ---- 算法型渐变：Turbo、Cubehelix ----
def palette_turbo(x: np.ndarray) -> np.ndarray:
    """Google Turbo colormap (approximation, vectorized)."""
    x = _clamp01(x.astype(np.float32))
    # Coefficients from Turbo paper/implementation
    a = np.array([
    ...
    X = np.stack([np.ones_like(x), x, x**2, x**3, x**4, x**5], axis=-1)
    rgb = X @ a.T
    return (np.clip(rgb, 0, 1) * 255).astype(np.uint8)


def palette_cubehelix(x: np.ndarray, start=0.5, rot=-1.5, gamma=1.0, sat=1.2) -> np.ndarray:
    """Cubehelix (Dave Green) implementation. start∈[0,3), rot: turns, gamma: intensity."""
    x = _clamp01(x.astype(np.float32))
    xg = x ** gamma
    a = sat * xg * (1 - xg) / 2.0
    phi = 2.0 * np.pi * (start / 3.0 + rot * x)
    r = xg + a * (-0.14861 * np.cos(phi) + 1.78277 * np.sin(phi))
    g = xg + a * (-0.29227 * np.cos(phi) - 0.90649 * np.sin(phi))
    b = xg + a * (1.97294 * np.cos(phi))
    rgb = np.stack((r, g, b), axis=-1)
    return (np.clip(rgb, 0, 1) * 255).astype(np.uint8)


def palette_cubehelix_default(x: np.ndarray) -> np.ndarray:
    return palette_cubehelix(x)


# ---- 预设好看的渐变 ----
palette_viridis = gradient_palette([
    (68/255, 1/255, 84/255),   # 深紫
    (59/255, 82/255, 139/255),
    (33/255, 144/255, 141/255),
    (94/255, 201/255, 98/255),
    (253/255, 231/255, 37/255),  # 黄
])

palette_inferno = gradient_palette([
    (0/255, 0/255, 4/255),
    (85/255, 15/255, 109/255),
    (187/255, 55/255, 84/255),
    (249/255, 142/255, 9/255),
    (252/255, 255/255, 164/255),
])

palette_magma = gradient_palette([
    (0/255, 0/255, 0/255),
    (84/255, 10/255, 85/255),
    (187/255, 55/255, 125/255),
    (249/255, 142/255, 66/255),
    (252/255, 253/255, 191/255),
])

palette_plasma = gradient_palette([
    (13/255, 8/255, 135/255),
    (126/255, 3/255, 167/255),
    (203/255, 71/255, 119/255),
    (248/255, 149/255, 64/255),
    (240/255, 249/255, 33/255),
])

palette_cividis = gradient_palette([
    (0/255, 32/255, 77/255),
    (40/255, 66/255, 115/255),
    (69/255, 100/255, 124/255),
    (102/255, 135/255, 118/255),
    (182/255, 190/255, 97/255),
])

palette_sunset = gradient_palette([
    (41/255, 10/255, 89/255),   # 深紫
    (120/255, 30/255, 150/255), # 紫
    (225/255, 50/255, 100/255), # 品红
    (255/255, 120/255, 50/255), # 橙
    (255/255, 220/255, 100/255) # 浅黄
], gamma=0.9)

palette_ice = gradient_palette([
    (0/255, 10/255, 30/255),  # 深蓝
    (0/255, 80/255, 160/255), # 蓝
    (0/255, 180/255, 200/255),# 青
    (180/255, 240/255, 255/255), # 淡青
    (255/255, 255/255, 255/255), # 白
], gamma=0.8)

palette_forest = gradient_palette([
    (5/255, 20/255, 5/255),    # 深绿
    (20/255, 70/255, 30/255),
    (60/255, 120/255, 60/255),
    (160/255, 200/255, 120/255),
    (230/255, 220/255, 170/255),
], gamma=0.9)

palette_neon = gradient_palette([
    (0, 0, 0),
    (0/255, 255/255, 180/255),  # 青绿
    (255/255, 0/255, 255/255),  # 紫红
...
PALETTES: dict[str, Callable[[np.ndarray], np.ndarray]] = {
    ...
    "cubehelix": palette_cubehelix_default,
